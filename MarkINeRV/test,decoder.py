...
##########3
    ...
    def read_stegofile():
        ...
        # steg_img2 = torch.load("D:\\HiNet-main\\HiNet-main\\Dataset\\DIV2K\\DIV2K_stego_HR\\tensor.pth")


        # 读取图像文件并转换为PyTorch张量
        steg_img2 = cv.imread("D:\\HiNet-main\\HiNet-main\\Dataset\\DIV2K\\DIV2K_stego_HR\\000.png")
        ...
        steg_img2 = transf2(steg_img2)
        steg_img2 = steg_img2.unsqueeze(dim=0)
        steg_img2 = steg_img2.to(device)
        # 打印图像张量的形状和数据类型
        print(steg_img2.shape)
        print(steg_img2.dtype)
        # steg_img2 = totensor(steg_img2)
        ...
        # steg_img2 = Image.open(image_path)
        # # steg_img2 = steg_img2.unsqueeze(dim=0)
        # trans = transforms.ToTensor()
        # steg_img2 = trans(steg_img2)
        # # steg_img2 = steg_img2.unsqueeze(dim=0)
        # steg_img2 = steg_img2.to(device)
        return steg_img2
    ...
    # # trans = T.Compose([
    # #     T.CenterCrop(c.cropsize_val),  # 在图片的中间区域进行裁剪
    #     T.ToTensor(),
...
###########
    ...
    with torch.no_grad():
        for i, data in enumerate(datasets.testloader):
            ...
            #####################
            steg_img2 = read_stegofile()
            output_steg = dwt(steg_img2)
