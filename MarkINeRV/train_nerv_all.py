...
def main():
    ...
    if args.debug:
        args.eval_freq = 1
        args.outf = os.path.join(os.getcwd(),'output/debug')
    else:
        args.outf = os.path.join(os.getcwd(), os.path.join("output",args.outf))
...
def train(local_rank, args):
    ...
    if local_rank in [0, None]:
        ...
        with open('{}/rank0.txt'.format(args.outf), 'a') as f:
            f.write(str(model) + '\n' + f'{param_str}\n')
            # args.outf,
        writer = SummaryWriter(os.path.join(param_str, 'tensorboard'))
    else:
        writer = None
